...
class CalendarClientController extends BaseController
{
    ...
    public function index(CalendarRequest $request)
    {
        ...
        $calendars = $this->repository            
            ...
            ->scopeQuery(function ($query) {
                return $query->whereUserId(user_id('client.web'))->whereUserType(user_type('client.web'))->orderBy('id', 'DESC');
            })->all();
    ...
    public function store(CalendarRequest $request)
    {
        try {
            ...
            $attributes['user_id'] = user_id('client.web');
            $attributes['user_type'] = user_type('client.web');
    ...
    public function calendarList(CalendarRequest $request)
    {
        ...
        $arr = $this->repository->scopeQuery(function($query){
                       return $query->where('status', '!=', 'Draft')
                        ->where(function ($q){              
                        return $q->whereUserId(user_id('client.web'))->whereUserType(user_type('client.web'));
                    });
                 })->all();
            ...
            ->scopeQuery(function ($query) {
                return $query->orderBy('id', 'DESC')->where(function($q){              
                        return $q->where(function($qry){
                          return  $qry->whereUserId(user_id('client.web'))->orWhere('assignee_id','=',user_id('client.web'));
                        });
                }); 
            })->all();
