...
class Trans
{
    ...
    public function __construct()
    {
        ...
        if (empty($supportedTrans[ $this->defaultTrans ])) {
            throw new UnsupportedTransException('Laravel default locale is not in the supportedTrans array.');
        }
    }
    ...
    public function setLocale($locale = null)
    {
        ...
        if (!empty($this->supportedTrans[ $locale ])) {
            $this->currentTrans = $locale;
        } else {
            ...
            // if we reached this point and hideDefaultTransInURL is true
            // we have to assume we are routing to a defaultTrans route.
            if ($this->hideDefaultTransInURL()) {
                $this->currentTrans = $this->defaultTrans;
            }
            // but if hideDefaultTransInURL is false, we have
            // to retrieve it from the browser...
            else {
                $this->currentTrans = $this->getCurrentTrans();
            }
        }

        $this->app->setLocale($this->currentTrans);

        // Regional locale such as de_DE, so formatLocalized works in Carbon
        $regional = $this->getCurrentTransRegional();
        if ($regional) {
     ...
     */
    public function setSupportedTrans($locales)
    {
        $this->supportedTrans = $locales;
    }
    ...
    public function getLocalizedURL($locale = null, $url = null, $attributes = [])
    {
        ...
        $url_locale = $this->getDefaultTrans();
        ...
        if (!$parsed_url || empty($parsed_url[ 'path' ])) {
            ...
            $path = $parsed_url[ 'path' ];
            foreach ($this->getSupportedTrans() as $localeCode => $lang) {
    ...
    public function getURLFromRouteNameTranslated($locale, $transKeyName, $attributes = [])
    {
        ...
        if (!is_string($locale)) {
            $locale = $this->getDefaultTrans();
        }

        $route = '';

        if (!($locale === $this->defaultTrans && $this->hideDefaultTransInURL())) {
            $route = '/'.$locale;
        }
     ...
     * @return array
     */
    public function getSupportedTrans()
    {
        if (!empty($this->supportedTrans)) {
     ...
     */
    public function getCurrentTransName()
    {
        return $this->supportedTrans[ $this->getCurrentTrans() ][ 'name' ];
    }
     ...
     */
    public function getCurrentTransNative()
    {
        return $this->supportedTrans[ $this->getCurrentTrans() ][ 'native' ];
    }
     ...
     */
    public function getCurrentTransDirection()
    {
        if (!empty($this->supportedTrans[ $this->getCurrentTrans() ][ 'dir' ])) {
            return $this->supportedTrans[ $this->getCurrentTrans() ][ 'dir' ];
        }

        switch ($this->getCurrentTransScript()) {
     ...
     */
    public function getCurrentTransScript()
    {
        return $this->supportedTrans[ $this->getCurrentTrans() ][ 'script' ];
    }
     ...
     */
    public function getCurrentTransNativeReading()
    {
        return $this->supportedTrans[ $this->getCurrentTrans() ][ 'native' ];
    }
     ...
     */
    public function getCurrentTrans()
    {
        ...
        if ($this->useAcceptLanguageHeader()) {
            $negotiator = new LanguageNegotiator($this->defaultTrans, $this->getSupportedTrans(), $this->request);
     ...
     */
    public function getCurrentTransRegional()
    {
        // need to check if it exists, since 'regional' has been added
        // after version 1.0.11 and existing users will not have it
        if (isset($this->supportedTrans[ $this->getCurrentTrans() ][ 'regional' ])) {
            return $this->supportedTrans[ $this->getCurrentTrans() ][ 'regional' ];
        } else {
            return;
        }
    }
    ...
    {
        return array_keys($this->supportedTrans);
    }
     ...
     */
    public function checkTransInSupportedTrans($locale)
    {
        $locales = $this->getSupportedTrans();
    ...
    public function getRouteNameFromAPath($path)
    {
        ...
        $path = str_replace('/'.$this->currentTrans.'/', '', $path);
        $path = trim($path, '/');
