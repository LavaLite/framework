    ...
    public function store(MessageRequest $request)
    {
        try {
            ...
            foreach ($mail_to as $user_id) {
                ...
                {
                    //draft
                    $draft['user_id'] = user_id($guard.'.web');
                    $draft['user_type'] = user_type($guard.'.web');
                    $draft['to'] = $user_id;
                    $draft['status'] = "Draft";
                    $message1 = $this->repository->create($draft);
                     ...
                     //sent
                    $sent['user_id'] = user_id($guard.'.web');
                    $sent['user_type'] = user_type($guard.'.web');
                    $sent['to'] = $user_id; 
                    $message = $this->repository->create($sent);

                    //inbox
                    $inbox['user_id'] =user_id($guard.'.web');
                    $inbox['user_type'] = user_type($guard.'.web');
                    $inbox['to'] = $user_id;
                    $inbox['status'] = "Inbox";
    ...
    public function destroy(MessageRequest $request, Message $message)
    {
        ...
        try {
            ...
            return response()->json([
                'message'  => $e->getMessage(),
                'code'     => 400,
                'redirect' => trans_url('client/message/message/' . $message->getRouteKey()),
            ], 400);
        }
    }
        ...
        $messages['data'] =  $this->repository->scopeQuery(function ($query) use ($status,$guard,$email) {
                ...
                ->where(function($query) use($status,$email){
                    if ($status == 'Inbox') {
                        ...
                        $query->whereUserId(user_id('client.web'))
                            ->whereUserType(user_type('client.web'));
                    }
                })
                ->whereStatus($status)
                ->orderBy('id', 'DESC');
