...
class TaskClientController extends BaseController
{
    ...
    public function store(TaskRequest $request)
    {
        try {
            ...
            $attributes['assigned_to'] = user_id('client.web');
        ...
        $tasks  = $this->repository
                    ->scopeQuery(function($query){
                            return  $query->where(function($q){
                                    $q->whereAssignedTo(user_id('client.web'))
                                    ->orWhere('user_id','=',user_id('client.web'));
                            })->orderBy('id','DESC');            
                    })->all(); 
