...
@section('heading')
<i class="fa fa-file-text-o"></i> {!! trans('user::user.role.name') !!} <small> {!! trans('cms.manage') !!} {!! trans('user::user.role.names') !!}</small>
@stop

@section('title')
{!! trans('user::user.role.names') !!}
@stop

@section('breadcrumb')
<ol class="breadcrumb">
    <li><a href="{!! trans_url('admin') !!}"><i class="fa fa-dashboard"></i> {!! trans('cms.home') !!} </a></li>
    <li class="active">{!! trans('user::user.role.names') !!}</li>
</ol>
@stop
...
<table id="user-role-list" class="table table-striped table-bordered">
    <thead>
        <th>{!! trans('user::user.role.label.name')!!}</th>
    </thead>
</table>
@stop
