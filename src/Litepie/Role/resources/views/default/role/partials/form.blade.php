                ...
                <ul>
                    @foreach($permissions as $package => $modules)
                        ...
                        @foreach($modules as $module => $permissions)
                          <li class="custom-control custom-checkbox" style="margin-left:-40px;">
                          <input name="sub[{{$package}}.{{$module}}]" class="custom-control-input" id="permissions_{{$package}}_{{$module}}" type="checkbox" {{ @array_key_exists($package. '.' . $module, $data['permissions']) ? 'checked' : '' }} value='1'>
                          <label class="custom-control-label" for="permissions_{{$package}}_{{$module}}">{{ucfirst($module)}}</label>
                            ...
                            <ul class="clearfix" style="padding-left:0px;">
                              ...
                              <li class="custom-control custom-checkbox" style="float:left; margin-right: 10px;">
                                  <input name="permissions[]" class="custom-control-input" id="permissions_{{$package}}_{{$module}}_{{$permission}}" type="checkbox" {{ @array_key_exists( $value, $data['permissions']) ? 'checked' : '' }} value='{{$value}}'>
